...
public class AILevelTwo : AILevelOne {
    ...
    //掃棋邏輯:
    //bool lfirst = true, lstop= false, rstop = false; int AllNum = 7;
    //先從左邊掃，掃到左右邊會因為撞到棋盤最邊緣或是撞到另一顏色的棋子而停止
    //但萬一掃下去都沒撞到怎辦?所以設置int AllNum = 7;最多就掃7個
    //int AllNum = 1;初始預設為1，只掃一個。再去遍歷慢慢掃，最多掃7個
    public override void CheckOneLine(int[] pos, int[] offset, int chess)
    {
        bool lfirst = true, lstop= false, rstop = false;
        int AllNum = 1; //AllNum最長為7，因為字串"_aaaaa_"
        string str = "a";//代表下的一個棋子
        int ri = offset[0], rj = offset[1];//右邊偏移量
        int li = -offset[0], lj = -offset[1];//左邊偏移量
